==================================================================
FlattenList: Flatten a list of lists in one line in Python
==================================================================
...
The boilerplate code provides a dummy ``main`` function that prints out the word 'Hello'::
    ...
    >> from flatten_list import main
    >> main()
    
When the package is installed via ``easy_install`` or ``pip`` this function will be bound to the ``flatten_list`` executable in the Python installation's ``bin`` directory (on Windows - the ``Scripts`` directory).
